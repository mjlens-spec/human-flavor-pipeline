...
## 0.8.0:内外部调研回灌 —— 声口落地 ＋ 实战盲区修补

0.7.x 把规则打磨到位,但 0.8 之前有个结构性空缺:**声口锚点是空模板**。Skill 设计上「有用户真文就必走 few-shot 对齐声口」是核心差异点,但 `style-anchors.md` / `exemplars.md` 的个人锚点一直没填。本版做内外部调研后集中落地。

- **内部资料提炼**:从本周真实写作语料(客户方案 / 事件营销方向稿 / 提案会议逐字稿)提炼用户声口画像(结论前置、短句砸判断、「→」递进、「她们」＋行为细节、callout 分层、不升华结尾),填进声口参数表与脱敏示范锚点。机密纪律:脱敏示范进公开 repo,客户真稿留本地。
- **实战盲区修补**:用真实稿反推出 Skill 规则与实际改写的 gap。最关键两条来自对本项目自己改过的稿的回看:(a) 改写补数据没标来源 → 新增【新增信息】标记;(b) 改写用了否定先行模板而个人层硬禁 → 松绑为策略分野例外。其余:营销圈内词白名单缺类、结构腔藏表格漏检、达人脚本缺专项声口、四字格 / 整数框架误杀。
- **外部调研借鉴**:GitHub 三路(通用 humanizer / 检测器信号 / 中文专项)。吸收 blader Voice Calibration 的声口参数化、cnki-aigc「挪动句法角色比换词有效」、检测器侧可量化指标方向(句长方差 / 四字格密度 / 整数框架,列为 P2 探索)。交叉核对确认语体矩阵、英文句法残留(`catalog/syntax.md`)、保留毛边等我们已有,不重复造轮子。
- **暂未纳入(留待后续)**:篇章级客观量化脚本(P2-1,会改评分底层逻辑,单独立项)、声口校准动态化、平台合规词层、实战回灌机制化。详见仓库根 `改进路线-本周内外部调研` 文档。

## 0.7.1:口播声口与可朗读校准

0.7.0 已经把「声口必走」立住,但 voice profile 仍偏书面稿。播客逐字稿、视频稿和演讲稿需要另一层判断:能不能被一个人自然说出来。本版补的是通用口播机制,不是某个节目的私有风格。

- **新增口播声口** —— 承重句短准,连接组织允许第一人称、反问、口头转场、适量软垫和自我修正。
- **软垫词按功能判断** —— 「其实 / 就是 / 我觉得」在书面稿里常是填充,在口播里可能是呼吸和转场;只有无信息堆叠才扣分。
- **承重句 / 连接组织拆分** —— 金句、判断、数字和事实是承重句,要准;解释、转场、回环和听众节奏是连接组织,可以松。
- **私有声口不入公开库** —— 具体主播的弹药库、节目母题、固定收尾、个人经历只适合放本地 `style-anchors.md` 或外部私有文档,公开 repo 只保留槽位和方法。
