...
- 用户要求润色、改写、直接改好或保存成稿:full。快速聊天和短句走轻档 full,报告压缩。
- 用户要求建议、局部审校或改法对照:review。正式长文只说「看看」且未说明修改意图时,先交付 review。
- 用户明确说「只检查 / 先评分」:detect。

用户已选定的交付方式优先于默认选择。载体正式或已有初稿不构成新的审批点。
...
4. **语体至多漂移一级**:用户已指定新的受众、载体或语气时据此调整,不受此限;只有这些要求存在影响结果的冲突时才提问。
...
按 `references/report-format.md` 交付。用户要求直接改好并保存时,完成授权范围内的修改和保存,不重复请求确认;写回后回读验证预期范围和保护区。用户只要求诊断或建议时,在该范围内交付。目标文件、替换范围或冲突存在实质性歧义时,先完成不受影响的部分,再询问。

保存授权不包含发送给他人、公开发布或扩大共享权限。涉及客户报价 / 未公开数据的稿件可以向被授权的协作对象定向流转、送审和交付;不得发给未授权对象,也不自动写入公开库。
