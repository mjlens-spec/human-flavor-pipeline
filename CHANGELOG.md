# Changelog

## 1.2.1 - 2026-09-24

去掉三处不必要的停顿确认。规则本身不变,改的是什么时候需要停下来问用户。

- `SKILL.md` 默认选择:用户要求润色、改写、直接改好或保存成稿时走 full,不再因为载体是正式文档、客户方案或长文先交付 review 再等确认;只说「看看」且未说明修改意图时仍先给 review。用户已选定的交付方式优先于默认选择
- `SKILL.md` 硬规则 4:用户已指定新的受众、载体或语气时据此调整,不再为跨级改写单独确认;只有这些要求存在影响结果的冲突时才提问
- `SKILL.md` 交付与写回:用户要求直接改好并保存时保存并回读,不重复请求确认;目标文件或替换范围有实质歧义时,先完成不受影响的部分再问。写明保存授权不包含发送、公开发布或扩大共享
- `SKILL-lite.md` 只同步版本号,单文件版流程不变

## 1.2.0 - 2026-09-23

补两条表达模式。逐条比对 [Humanizer-zh](https://github.com/op7418/Humanizer-zh) 2026-09-23 修订(f4518a8)的 31 条模式后,28 条已有对应规则,以下两条在本项目中没有覆盖。条目文字与例句为本项目重写,未复制上游文本。

- `patterns/catalog/patterns.md` 新增 #5a 模糊关联(对应上游 #14):同一段已写明具体角色或关系时,删「与……有着密切联系 / 息息相关 / 渊源颇深」式笼统说法;原文没写明关系时保留概括,补写具体关系按新增事实判失败。统计意义上的「显著相关」和如实交代信息边界的说法不触发
- `patterns/catalog/patterns.md` 新增 #27b 与假想敌辩论(对应上游 #5):被反驳的看法在原文没有出现、也没有出处,且删掉辩护句后主张不变,两条同时满足才触发;回应有出处的真实质疑、说明限制条件、否定部分限定主张强度时保留。写明与 #48 否定对举、27a 假坦诚开场的分界
- `patterns/banned-words.md`:A 组补「模糊关联」,C 组补「与假想敌辩论」
- `references/scoring.md`:模糊关联按内容注水同档 +5;与假想敌辩论并入「设问自答、假坦诚钩子」档 +6
- `patterns/precision-rules.json` documentation 契约钉住两条新条目(开放类判断留在散文层,不加正则规则);`SKILL-lite.md` 同步单文件版
- 上游 #18「回避是 / 有」复核后维持不收录,理由补入 `patterns.md`「故意不收录的规则」
- `UPSTREAM.lock`、`CREDITS.md`、`references/design-notes.md` 登记 Humanizer-zh 固定 commit 与借鉴范围

## 1.1.0 - 2026-07-22

自造词入诊断。来自真实工作 session 的观察:AI 方案爱发明比喻性说法顶替行业通行术语(把「投放产品 / 广告产品」写成「投放路径」),同行和客户对不上概念。0.9.1 修过本项目自己的这类词(「场景门」→「适用场景匹配」),这版把同一判断上升为通用审核标准:有专用名词或通行说法的,必须用专用名词或通行说法。

- `patterns/banned-words.md` B 组新增开放类「自造术语」:词每次都是新的,固定词表抓不住,判据是「有现成说法不用」;三条判定(语义已被通行术语覆盖 / 文中无定义且非复用命名 / 给得出对应通行说法)同时满足才触发,给不出替换词不判命中
- `patterns/catalog/patterns.md` 新增 #9a 深查条目:原文 / 改后对照(投放路径→投放产品、效率线→阈值)、与 #9 同义词轮换和 J 组黑话的分界、语体限定(客户方案 / 报告等交付物从严,内部脑暴临时代号不扫)
- 防误伤边界:平台产品名与行业沉淀术语、有定义句且被复用的自研方法论命名(「荷花模型」「海陆空」等公式化命名法,见 `corpus/wording-glossary.md`)、作者 / 团队约定用语、确无通行说法的新事物命名,均放行
- `references/scoring.md`:自造术语每个去重 cluster +6,与 Tier 1 共用词汇类封顶 24;命中必须给「原词 → 通行说法」对照
- `references/editing-protocol.md` 词汇步骤同步:有通行术语的概念还原成通行说法,不保留自造词
- `patterns/precision-rules.json` documentation 契约钉住新条目(开放类判断留在散文层,不加正则规则);`SKILL-lite.md` 同步单文件版
- `profiles/lens.md` 补维护者个人雷区(「效率线 / 双引擎 / 半案」),只在明确启用 lens Profile 时生效,不进通用层

## 1.0.0 - 2026-07-13

可信编辑版。0.7–0.9 解决了「知道什么叫 AI 味」;1.0 开始证明「确实改对了,而且没有把别人的话写成作者经历」。

- 新增 **review 模式**:评分 + 热区 + 3–8 处局部 Before / After,正式文档默认先 review
- 新增 `scripts/check-fact-integrity.py`:数字、金额、日期、版本、引语、代码、占位符 missing / changed / added 硬闸,20 条确定性 fixture
- 普通 transform **禁止新增事实**;标「待核实」也不能进入终稿。外部研究必须先给来源,并入输入后再编辑
- Golden 改为结构化 JSONL + 可运行盲化 A/B runner,补 G7 / G9 的事实规则冲突
- 新增 `profiles/default.md` / `profiles/lens.md`:公开安装不再自动继承维护者笔调
- 新增 Marketing Agency Domain Pack 与 corpus 定向检索路由,不再全量加载公司语料
- 人味质量分重构为「判断与动机 / 具体与保真 / 笔调连续性 / 节奏可读性 / 节制与信任」
- `SKILL.md` 收缩为渐进加载编排层;详细编辑、报告和反馈回灌移入 references

## 0.9.2 - 2026-07-13

安装与分发修复版。仓库规则完整,但旧的全局安装只复制根目录 `SKILL.md`,运行时缺少它引用的 `patterns/`、`references/` 与 `corpus/`;本版先修真实安装面,为 1.0 的可信编辑闭环打底。

- `SKILL.md` frontmatter 改为规范字段:`license: MIT` ＋ `metadata.version`
- 新增 `skills/human-flavor-pipeline/` 完整可安装包与安装后资源检查
- Claude Code / Codex 统一消费同一个 `SKILL.md`;根目录 `AGENTS.md` 改为薄的仓库维护说明
- 新增 `agents/openai.yaml`、GitHub Actions 与 Skill Creator 标准校验
- 高风险稿件的边界改为授权流转:可定向送审和交付,不得发给未授权对象,不自动进入公开库

## 0.9.1 - 2026-07-01

术语去生硬化 + README 重排。用户觉得几个核心术语翻译腔偏重、README 版本记录堆成一段读不清楚,这版一起打磨。

- **术语替换(全仓库,含 corpus/、AGENTS.md 同步重新生成)**:「声口」→「笔调」(中文里本来就指写作呈现出的口吻,如"犀利笔调",比"声口"更顺,也避免跟本项目已用于负面 AI 味模式的"腔"字撞车);「场景门」→「适用场景匹配」;「四维门控」→「四项维度权衡」
- **README 版本记录改版**:原来挤成一段引用块的 `> vX.X 要点` 全部拆成独立列表项,并按最新在前重排(原是最旧在前);标题改叫「版本沿革」
- **README 语言打磨**:清掉正文里的破折号堆砌与直弯引号混用,English summary 里的三段式列举句改写成更口语的短句
- 全局 `CLAUDE.md` 写作偏好同步更新:破折号不再是刚性禁用项,少量恰当使用不算 AI 味信号;`patterns/style-anchors.md` 已提炼笔调参数表同步松绑

## 0.9.0 - 2026-07-01

新增 `corpus/` 项目语料库 —— 首次从头条易公司知识库(飞书《华东知识库 2026.5》)系统性提炼营销代理行业语料,是本管线首次收录「公司 / 团队级」参考资料(此前 `exemplars.md` 只收个人笔调锚点)。

- **新增 `corpus/case-library.md`**:「头条易华东项目库」多维表格全量 81 条真实客户项目,提炼「适用场景 / 核心策略 / 内容摘要」等方法论文本
- **新增 `corpus/company-playbook.md`**:公司通案 / 行业通案 / 平台业务通案 / 流程规范,18 份源文档(含 6 份旗舰级 PPTX 经 soffice 转 PDF 精读)
- **新增 `corpus/cross-industry-methodology.md`**:跨行业方法论沉淀,8 份源文档
- **新增 `corpus/wording-glossary.md`**:技术性 / 措辞性文案词汇表,汇总去重版,风险最低、供未来并入 `catalog/whitelists.md`
- **脱敏方法论固化**:机械替换 → 多路独立 agent 通读复核(不依赖清单,专防「脱敏说明自曝品牌名」与「XX 可保留的例外辩解」两类失败模式)→ 全量交叉核查 → **用户本人逐字通读原文终审**,详见 `corpus/README.md`「脱敏流程」
- **一份来源材料(行业案例补充语料)** 经用户审核判定含不易把握的敏感内容,明确决定保留本地私有、不进公开仓库,不在本次发布范围
- `SKILL.md`「数据层与参考」补充 `corpus/` 指引,选读、不默认全量加载

## 0.8.5 - 2026-07-01

达人业务语料回灌 ＋ 口播新反面模式。来源:「头条易 TTY」星图达人招采方案与「何以至此」播客新增逐字稿(两路 Sonnet 子 agent 并行扫描,与 0.8.0 那轮互补、不重复)。

- **达人业务黑话补白名单**:`catalog/whitelists.md` 新增「达人业务管理」类(四步筛选漏斗 / 媒介金字塔分层 / 二级代理 / 转委托 / 分包 / 爆文黄金公式),扩充投放媒介(FS 投放 / 看后搜 / 星智放大)、达人内容(领跑 / 核腰 / 核心 / 铺量 / 开白)、电商运营(A1-A5 人群资产 / 种子人群)三类
- **表格自证套路不计 AI 味**:`channel-presets.md` 乙方腔边界 ＋ `banned-words.md` C 组新增说明 —— 「为什么非用 X」论证句 ＋ 红线自查勾选行是功能性写法,区别于「结构腔藏在表格里」的空对称
- **多层结构叠加新判据**:`banned-words.md` C 组 ＋ `scoring.md` —— 单层排比 / 整数框架各自合规不代表叠加合规,多套四分法同时嵌套仍计一次 G 组均质化分,处理方式是打散呈现节奏而非删结构
- **口播新增两条反面模式**:`voice-profiles.md` 口播反面新增「材料引入式冷开场」禁令(不得用第三方转述代替亲历起手)与「系列回扣不硬编期号」安全规则(防 off-by-one),`SKILL.md` 阶段 3 口播校准同步
- **达人脚本六段式骨架**:`exemplars.md` 新增可执行范例(前 3 秒钩子→起→转→合→CTA ×画面 / 口播字幕 / 产品露出),`voice-profiles.md` 达人脚本亚型加交叉引用
- **口播笔调锚点补充**:`exemplars.md` 个人锚点新增脱敏引语,同时示范「不是 X 而是 Y」单次否定先行给审美下定义的合法用法
- **golden 新增 G9 / G10**:验证冷开场与多层结构叠加两条新规则,原创脱敏构造,不引用真实节目内容
- 「审计他人 AI 草稿」场景评估后**不**单列新模式:判定与「改写自己的稿子」形态一致,detect / full 已够用,不拆两套体系

## 0.8.0 - 2026-06-28

内外部调研集大成落地:笔调锚点填实 ＋ 营销圈内词白名单 ＋ 否定先行个人层松绑 ＋ 新增信息标记 ＋ 达人脚本笔调 ＋ 翻译腔操作层上提。来源:本周飞书写作语料(方案 / 口播逐字稿)内部提炼 ＋ GitHub 同类项目外部调研(通用 humanizer / 检测器信号 / 中文专项三路)。

- **笔调锚点填实**:`style-anchors.md` 增「已提炼笔调参数」表(脱敏 fallback 锚),`exemplars.md` 第三节补脱敏笔调示范锚点;真稿留本地、勿提交,真稿优先级高于脱敏示范
- **营销 / 媒介圈内词白名单**:`catalog/whitelists.md` 新增第五类(投流 / 千川 / 星图 / KFS / 水下 / 卡审 / CPE / CPM 等),作 J 组与 D 组硬例外,防洗掉乙方专业笔调
- **否定先行个人层松绑**:`user-taste.md` 从一刀切硬禁改为「策略分野例外」,与 0.7.0 `negation_first_cluster` 密度门控对齐;`banned-words.md` 同步
- **新增信息标记(修 CC 反推)**:阶段 4 清点改写新增的事实,打磨报告增【新增信息】项,`scoring.md` 增「新增信息风险」 —— 凡补进原文没有的数字 / 数据 / 出处一律标「待核实来源」,不静默植入
- **达人 / 博主脚本笔调**:`voice-profiles.md` 口播笔调增亚型,`channel-presets.md` 文体表增行(直给 / CTA / 画面可拍,区别于播客逐字稿)
- **C 组防误杀 ＋ 补漏检**:三段排比 / 整数框架 / 四字格按文体阈值判,各项有实质差异不触发;表格 / grid 三列对照同样适用结构腔判断(补结构腔藏表格的漏检)
- **B 组口语变体豁免**:口语原生「但是 / 然后 / 所以」与书面「然而 / 因此 / 此外」分开判,口播 / 对话体不计分
- **F 组翻译腔操作层上提**:`banned-words.md` F 组补介词前置 / 令人+形容词 / 定语堆叠 / 作为一个 X 四条,接 `catalog/syntax.md` 深查层
- **内部对齐稿 / 方向稿语体**:`channel-presets.md` ＋ `scoring.md` 声明工作化口语、括号备注、TODO 不计 AI 味
- **golden 模型级评测**:`tests/golden/cases.md` 新增 3 例(达人脚本防文艺化、改写新增信息须标待核实、整数框架实质差异不误杀)
- 保持 precision 契约与 floor 规则不变;本次是数据层与笔调规则更新

## 0.7.1 - 2026-06-23

口播 / 播客逐字稿笔调补强,目标是防止 pipeline 把可说出来的稿子洗成书面稿。

- 新增 **口播(Spoken)** voice profile:承重句短准,连接组织允许第一人称、反问、口头转场、适量软垫和自我修正
- `SKILL.md` 阶段 3 新增口播校准:区分承重句与连接组织,同时明确不得编造亲历、数字、口头禅或固定收尾
- `patterns/banned-words.md` 与 `references/scoring.md` 增加口播软垫例外:有听觉功能的「其实 / 就是 / 我觉得」不按裸字符串扣分
- `patterns/style-anchors.md` 增加主播 / 口播锚点槽位,提醒私有节目素材留在本地,不要提交到公开 repo
- `tests/golden/cases.md` 增加口播逐字稿模型级评测用例,验证事实保真、软垫保留和防书面化
- 保持 29 条 precision 契约不变;本次是模型行为与笔调规则更新

## 0.7.0 - 2026-06-20

对抗 0.6 精度偏置的反向加固:补召回护栏、否定先行松绑、单一真相源、模型级评测、外部锚。

- 新增**检测下限 floor**(`precision-rules.json` 4 条无例外规则:开场烘托 / 收尾套话 / 空强调 / PR 黑话)＋ **召回测试** `tests/fixtures/19-recall-floor.jsonl`;护栏从「只防误报」变「精度 + 召回」双向(check-precision.py 加载 18+19,36 用例:20 防误报 + 16 正样本)
- **否定先行硬禁 → 密度门控**:`不是X而是Y` 单次放行,同段 ≥2 次才罚(`negation_first_cluster`),`user-taste.md` 同步松绑
- **单一真相源**:散文层对模型 canonical,`precision-rules.json` 加 `canonical_source` ＋ `calibration` 字段;SKILL/banned-words 标明
- **护栏正名 ＋ 模型级评测**:`check-precision.py` 改述为 guardrail(绿≠skill 正确);新增 `tests/golden/`(before→after ＋ 五维 rubric)
- **外部权威锚**:Wikipedia「Signs of AI Writing」背书模式组;self-audit 终极一问借自 best-humanizer-handbook
- **笔调必走**:有个人锚点时「写出自己的文风」从可选升为必走
- **词表校准**字段(应对模型迭代新味)＋ **`SKILL-lite.md`** 单文件变体(ChatGPT / Gemini 等)
- 调研:GitHub(best-humanizer-handbook 等)、小红书(头条去味工作流)

## 0.6.0 - 2026-06-18

精确率优先的规则校准,重点降低合法中文、专业文体与工作流残留的误报。

- 门检改为「改写必要性低 / 中 / 高」,不再猜测作者身份
- AI 味、人味、个人偏好、事实 / 来源风险、格式 / 载体风险拆轴;I/J 不重复计入 AI 味
- 载体、受众、文体、笔调拆开判断,Notion / 飞书 / 公众号不再绑定固定文体
- 破折号改为功能与密度判断,删除无语料支持的「真人基线为 0」
- 技术英语服从团队约定、行业惯例与受众;泛称代词、准确业务术语不做裸字符串命中
- UTM、Markdown 与模板占位符改列工作流 / 格式风险,不再作为 AI 来源证据
- 新增假坦诚、格言公式、连续短句造戏剧感、标题后复述、变更叙事、助手尾巴等模式,并补相邻反例
- 删除无法核验的「Tim Cook 2026 年告别信」统计与示例
- 新增 `UPSTREAM.lock` 固定 qu-ai-wei 与 blader/humanizer commit
- 新增 `patterns/precision-rules.json` production 规则清单;29 条 precision 契约直接加载真实阈值、上下文与例外
- precision 新增相邻负样本,并修复学术样本「无强命中却强制判 AI」与版本同步检查

## 0.5.0 — 2026-06-18

实战回灌(一次真实客户方案)＋ 范文库 ＋ 个人禁忌层。

- 新增 **J 组江湖气 / 互联网黑话**(`patterns/banned-words.md`):军事 / 体育比喻 ＋ 阿里腔黑话 ＋ 替换速查表
- 新增 **个人禁忌层** `patterns/user-taste.md`:用户点名禁词 ＋ 手动改稿习惯成文,优先级最高
- 新增 **范文库** `patterns/exemplars.md`:蒸馏人味正面规则 ＋ 原创脱敏短范文锚点(不含真实品牌案例、不引第三方原文)＋ 个人锚点模板(机密不入库)
- `channel-presets.md` 增**乙方腔好坏边界**:deck 结构不扣分,只去空话型乙方腔
- `scoring.md` 增 J 组 / 个人禁忌计分、**防跨词假阳性**、乙方腔不计入
- `SKILL.md` 增门检分流(底子好→定点清理)、**写回前过目 checkpoint**、机密保护
- `CREDITS.md` 增范文短引用的著作权法第 24 条声明与署名
- 重新生成 `AGENTS.md`

## 0.4.0 — 2026-06-17

跨工具兼容:Claude Code ＋ Codex 双入口。

- 新增 `AGENTS.md`(Codex 入口),由 `SKILL.md` 经 `scripts/build-agents.sh` 同源生成,剥离 Claude 专有 frontmatter、保留正文与数据层引用 —— 单一事实来源,不漂移
- `SKILL.md` / `references/self-audit.md` 的子 agent 复核改为工具无关措辞(Claude Code 用 Task 工具 / Codex 用其子 agent 机制)
- README 增「安装与使用」双工具说明(Claude Code skill ＋ Codex AGENTS.md)
- `patterns/` 与 `references/` 数据层保持工具无关,两端共享

## 0.3.0 — 2026-06-17

深度无损合并本地 `qu-ai-wei`(MIT,@LifelongLazyLearner),消重只留一个 humanizer skill。

- 并入 `patterns/catalog/` —— qu-ai-wei 的 961 行 51 条 verbatim 模式库、白名单、标点 / 句法、平台模式、品牌笔调、参考模型(作深查层,`banned-words.md` 拿不准时下钻)
- 并入 `tests/` —— 17 组 fixtures ＋ baseline / after 快照(作回归测试集)
- 新增 `CREDITS.md` 与 `patterns/catalog/LICENSE.qu-ai-wei`,保留 MIT 署名
- `banned-words.md` / `SKILL.md` 接线深查层;`design-notes.md` 记录合并
- 原 `qu-ai-wei` 从活跃 skills 归档至 `~/.claude/skills-archive/`

## 0.2.0 — 2026-06-17

集大成升级 —— 吸收 humanizer 类与 qu-ai-wei 类两条线的精华。

新增:
- **适用场景匹配**:快速 / 聊天 vs 正式交付,默认强度与报告粒度不同
- **语体阶梯 ＋ 九种语体**:输出至多偏移一格,防跨级洗坏
- **笔调(voice profiles)**:直白 / 温和 / 犀利 / 技术 / 叙事,「句长＋用词＋结构」捆绑(`patterns/voice-profiles.md`)
- **六大主动润色动作**:动词强化 / 节奏重塑 / 填充删除 / 抽象落具体 / 语序归位 / 语体匹配
- **AI 不敢写测试 ＋ 整篇五问自检 ＋ 空句检测 ＋ 门检判定**(`references/self-audit.md`)
- **人味质量分 0–50** 五维矩阵,与 AI 味分双轴并看(`references/scoring.md`)
- **实战样例**:改前 / 改后对照,含特稿「不该过度改」反例(`references/examples.md`)
- 词典重组为 **A–I 模式分组** ＋ 三级强度(`patterns/banned-words.md`)
- 校验改为**事实优先的两遍读**(先保真,后查 AI 腔)
- 底线原则 7 → 8 条

## 0.1.0 — 2026-06-17

首版。中文去 AI 味六阶段流水线:门检 → 诊断打分 → 剥离 → 注入人味 → 校验 → 报告交付。两种模式、三级词典、渠道预设、风格锚点、AI 味分、保护区、有界迭代。
